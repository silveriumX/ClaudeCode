#!/usr/bin/env python3
import os
from pathlib import Path

from dotenv import load_dotenv

load_dotenv(Path(__file__).resolve().parents[1] / ".env")

import base64, io, sys
if sys.platform == 'win32':
    sys.stdout = io.TextIOWrapper(sys.stdout.buffer, encoding='utf-8')
import paramiko

c = paramiko.SSHClient()
c.set_missing_host_key_policy(paramiko.AutoAddPolicy())
c.connect(os.getenv("VPS_WIN_HOST"), username='Administrator', password=os.getenv("VPS_WIN_PASSWORD"), timeout=10, look_for_keys=False)

cmd = r'''
# Быстрый TCP тест
$tcp = Test-NetConnection -ComputerName {os.getenv("PROXY_HOST")} -Port 10000 -WarningAction SilentlyContinue
Write-Output "TCP to {os.getenv("PROXY_HOST")}:10000 = $($tcp.TcpTestSucceeded)"

# Проверить маршруты
Write-Output ""
Write-Output "Routes to {os.getenv("PROXY_HOST")}:"
route print | Select-String "185.162"

# Проверить что NAT не ломает
Write-Output ""
Write-Output "NAT:"
Get-NetNat | Select-Object Name, InternalIPInterfaceAddressPrefix
'''

enc = base64.b64encode(cmd.encode('utf-16le')).decode()
_, o, _ = c.exec_command(f'powershell.exe -NoProfile -NonInteractive -EncodedCommand {enc}', timeout=30)
print(o.read().decode('utf-8', errors='ignore'))
c.close()
