"""
VPS Connection Utility for MedicalDocBot
=========================================
...
REMOTE_DIR = os.getenv("VPS_REMOTE_DIR", "/root/medical_doc_bot")
PROCESS_GREP = "medical_doc_bot/bot.py"
...
def bot_status():
    print("=== Bot Status ===")
    run_ssh(f"ps aux | grep '{PROCESS_GREP}' | grep -v grep || echo 'Bot is NOT running'")
    print("\n=== Uptime / RAM / Disk ===")
...
def bot_restart():
    ...
    run_ssh(
        f"cd {REMOTE_DIR} && "
        f"nohup python3 {REMOTE_DIR}/bot.py >> bot.log 2>> bot_error.log </dev/null & "
