import os
import asyncio
from datetime import datetime
from pathlib import Path

from aiogram import Bot, Dispatcher, types
from aiogram.filters import Command
from aiogram.enums import ChatType
from openai import OpenAI

# === НАСТРОЙКИ ===
from dotenv import load_dotenv
load_dotenv()
TELEGRAM_TOKEN = os.environ["TELEGRAM_TOKEN_PUBLIC"]
OPENAI_API_KEY = os.environ["OPENAI_API_KEY"]

# Корневая папка для сохранения транскрибаций (на сервере)
BASE_DIR = Path.home() / "transcriptions_public"
BASE_DIR.mkdir(exist_ok=True)

# === ИНИЦИАЛИЗАЦИЯ ===
bot = Bot(token=TELEGRAM_TOKEN)
dp = Dispatcher()
openai_client = OpenAI(api_key=OPENAI_API_KEY)


def get_user_directory(user_id: int, username: str = None, chat_title: str = None, chat_type: str = "private") -> Path:
    """Определяет папку для сохранения в зависимости от источника сообщения"""

    if chat_type in ["group", "supergroup"]:
        # Групповой чат - папка по названию чата
        safe_chat_name = "".join(c if c.isalnum() or c in (' ', '-', '_') else '_' for c in (chat_title or f"chat_{user_id}"))
        user_dir = BASE_DIR / "Чаты" / safe_chat_name
    else:
        # Личные сообщения - папка по username или ID
        folder_name = username if username else f"user_{user_id}"
        user_dir = BASE_DIR / folder_name

    user_dir.mkdir(parents=True, exist_ok=True)
    return user_dir


def get_today_file(user_dir: Path) -> Path:
    """Возвращает путь к файлу транскрибаций за сегодня"""
    today = datetime.now().strftime("%Y-%m-%d")
    return user_dir / f"{today}.md"


def append_transcription(user_dir: Path, text: str, username: str = None):
    """Добавляет транскрибацию в файл дня"""
    file_path = get_today_file(user_dir)
    time_now = datetime.now().strftime("%H:%M")

    # Создаём файл с заголовком если не существует
    if not file_path.exists():
        today = datetime.now().strftime("%d.%m.%Y")
        header = f"# Транскрибации за {today}\n\n"
        file_path.write_text(header, encoding="utf-8")

    # Добавляем транскрибацию с временной меткой
    with open(file_path, "a", encoding="utf-8") as f:
        if username:
            f.write(f"### {time_now} — @{username}\n{text}\n\n")
        else:
            f.write(f"### {time_now}\n{text}\n\n")


async def transcribe_voice(file_path: str) -> str:
    """Транскрибирует голосовое через Whisper"""
    with open(file_path, "rb") as audio_file:
        transcript = openai_client.audio.transcriptions.create(
            model="whisper-1",
            file=audio_file,
            language="ru"
        )
    return transcript.text


@dp.message(Command("start"))
async def cmd_start(message: types.Message):
    # В группах не отвечаем на команды
    if message.chat.type in [ChatType.GROUP, ChatType.SUPERGROUP]:
        return

    await message.answer(
        "Привет! Я транскрибирую голосовые сообщения.\n\n"
        "📌 Отправь голосовое → получишь текст\n"
        "📌 Работаю в личке и в группах\n\n"
        "Просто и быстро 🎤✨"
    )


@dp.message(lambda m: m.voice is not None)
async def handle_voice(message: types.Message):
    """Обрабатывает голосовые сообщения везде (личка + группы)"""

    # Определяем директорию для логирования
    user_dir = get_user_directory(
        message.from_user.id,
        message.from_user.username,
        message.chat.title,
        message.chat.type
    )

    # В личке показываем индикатор
    status_msg = None
    if message.chat.type == ChatType.PRIVATE:
        status_msg = await message.answer("🎤 Транскрибирую...")

    # Скачиваем голосовое
    file = await bot.get_file(message.voice.file_id)
    temp_path = f"/tmp/voice_{message.message_id}.ogg"
    await bot.download_file(file.file_path, temp_path)

    try:
        # Транскрибируем
        text = await transcribe_voice(temp_path)

        # Сохраняем для логирования (пользователи не имеют доступа)
        append_transcription(user_dir, text, message.from_user.username)

        # Отвечаем только транскрибацией - ничего лишнего
        await message.reply(text)

        # Удаляем индикатор в личке
        if status_msg:
            await status_msg.delete()

    except Exception as e:
        error_text = f"❌ Ошибка транскрибации"
        if status_msg:
            await status_msg.edit_text(error_text)
        else:
            await message.reply(error_text)
    finally:
        # Удаляем временный файл
        if os.path.exists(temp_path):
            os.remove(temp_path)


async def main():
    print("Публичный бот (simple) запущен на сервере...")
    print(f"Файлы логируются в: {BASE_DIR}")
    print("Функции: только транскрибация голосовых")
    await dp.start_polling(bot)


if __name__ == "__main__":
    asyncio.run(main())
