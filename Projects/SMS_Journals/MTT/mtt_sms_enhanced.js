...
function onOpen() {
  SpreadsheetApp.getUi()
    ...
    .addItem('🐛 DEBUG: Детальная загрузка', 'debugLoadHistory')
    .addItem('⚡ Принудительная загрузка (50 SMS)', 'forceLoadHistory')
    .addSeparator()
    .addItem('⏱️ Включить автозагрузку (каждые 5 мин)', 'enablePolling')
    .addItem('⏹️ Выключить автозагрузку', 'disablePolling')
    .addSeparator()
    .addItem('🔗 Показать URL веб-хука (Web App)', 'showWebAppUrl')
    .addItem('🔍 Диагностика веб-хука', 'diagnoseWebhook')
    .addItem('🔬 Расширенная диагностика API', 'diagnoseAPI')
...
function diagnoseAPI() {
  ...
  let report = '🔍 ДИАГНОСТИКА API МТТ\n\n';

  // 1. Проверка токена
  report += '1️⃣ Проверка токена:\n';
  if (!token) {
    report += '❌ MSAPI_TOKEN не задан!\n\n';
    ui.alert(report);
    return;
  }
  report += `✅ Токен задан (длина: ${token.length} символов)\n\n`;

  // 2. Проверка GetNumbers
  report += '2️⃣ Тест GetNumbers (список номеров):\n';
  try {
    const url = `${MTT_BASE}/v1/GetNumbers`;
    const resp = UrlFetchApp.fetch(url, {
      method: 'post',
      ...
      headers: { 'Authorization': `Bearer ${token}` },
      muteHttpExceptions: true,
      payload: JSON.stringify({ limit: 10, offset: 0 }),
    });
    const code = resp.getResponseCode();
    report += `HTTP: ${code}\n`;
    ...
    if (code === 200) {
      ...
      report += `✅ Успешно! Найдено номеров: ${count}\n`;
      if (count > 0) {
        report += `Первый номер: ${data.numbers[0].number}\n`;
      }
    } else {
      report += `❌ Ошибка: ${resp.getContentText().slice(0, 200)}\n`;
    }
  } catch (e) {
    report += `❌ Исключение: ${e.message}\n`;
  }
  report += '\n';

  // 3. Проверка GetMessagesHistoryList
  report += '3️⃣ Тест GetMessagesHistoryList (история SMS):\n';
  if (!customer) {
    report += '⚠️ CUSTOMER_NAME не задан (обязателен для истории)\n\n';
  } else {
    report += `Лицевой счёт: ${customer}\n`;
    try {
      const url = `${MTT_BASE}/v1/GetMessagesHistoryList`;
      const body = {
        customer_name: customer,
      ...
      if (code === 200) {
        ...
        report += `✅ Успешно! Найдено SMS: ${list.length}\n`;

        if (list.length > 0) {
          const last = list[0];
          report += `\nПоследнее SMS:\n`;
          report += `  Дата: ${last.event_date}\n`;
          report += `  Номер: ${last.number}\n`;
          report += `  От: ${last.sender}\n`;
          report += `  Текст: ${(last.text || '').slice(0, 50)}...\n`;
        } else {
          report += '\n⚠️ История пуста! Возможные причины:\n';
          report += '  • На номера не приходили SMS\n';
          report += '  • Неверный CUSTOMER_NAME\n';
          report += '  • SMS старше, чем хранит API\n';
        }
      } else {
        report += `❌ Ошибка: ${resp.getContentText().slice(0, 200)}\n`;
      }
  ...
  // 4. Проверка фильтров
  report += '4️⃣ Текущие фильтры истории:\n';
  const since = props.getProperty('HISTORY_SINCE');
  const until = props.getProperty('HISTORY_UNTIL');
  const number = props.getProperty('HISTORY_NUMBER');
  const direction = props.getProperty('HISTORY_DIRECTION');

  if (!since && !until && !number && !direction) {
    report += '  (нет фильтров - загружаются все SMS)\n';
  } else {
    if (since) report += `  HISTORY_SINCE: ${since}\n`;
    if (until) report += `  HISTORY_UNTIL: ${until}\n`;
    if (number) report += `  HISTORY_NUMBER: ${number}\n`;
    if (direction) report += `  HISTORY_DIRECTION: ${direction}\n`;
  }
...
/** ===================== DEBUG: ДЕТАЛЬНАЯ ЗАГРУЗКА ===================== */
function debugLoadHistory() {
  ensureSheets_();
  const props = PropertiesService.getScriptProperties();
  const token = props.getProperty('MSAPI_TOKEN');
  const customer = props.getProperty('CUSTOMER_NAME');
  ...
  if (!token) throw new Error('Нет MSAPI_TOKEN.');
  if (!customer) throw new Error('Нет CUSTOMER_NAME.');
  ...
  // Создаём лист DEBUG
  const ss = SpreadsheetApp.getActive();
  let debugSh = ss.getSheetByName('DEBUG История');
  if (!debugSh) {
    debugSh = ss.insertSheet('DEBUG История');
  }
  debugSh.clear();

  // Заголовки
  debugSh.getRange(1, 1, 1, 8).setValues([[
    'Дата API', 'Номер (API)', 'Отправитель (API)', 'Текст (API)',
    'Direction', 'Дата обработано', 'TO обработано', 'FROM обработано'
  ]]);
  ...
  for (const m of list) {
    ...
    const processedTime = normalizeTime_(m.event_date);
  ...
  // Записываем в DEBUG
  if (debugRows.length) {
    debugSh.getRange(2, 1, debugRows.length, 8).setValues(debugRows);
  }
  debugSh.setFrozenRows(1);
  debugSh.autoResizeColumns(1, 8);

  // Теперь пробуем добавить в журнал
  const added = appendToLogNoDup_(prepared);

  ss.setActiveSheet(debugSh);
  SpreadsheetApp.getUi().alert(
    `🔍 DEBUG РЕЗУЛЬТАТЫ:\n\n` +
    `📡 Получено из API: ${list.length} SMS\n` +
    `✅ Прошло фильтр (to/from/text): ${prepared.length}\n` +
    `📝 Добавлено в журнал (без дублей): ${added}\n\n` +
    `Откройте лист "DEBUG История" для деталей.\n\n` +
    `Если "Добавлено = 0", значит все SMS уже есть в журнале.`
  );
}

/** ===================== ПРИНУДИТЕЛЬНАЯ ЗАГРУЗКА (БЕЗ ПРОВЕРКИ ДУБЛЕЙ) ===================== */
function forceLoadHistory() {
  ...
  SpreadsheetApp.getUi().alert(
    `⚡ ПРИНУДИТЕЛЬНО ДОБАВЛЕНО: ${rows.length} записей\n\n` +
    `(без проверки дублей - могут быть повторы!)\n\n` +
    `Проверьте лист "Журнал СМС"`
  );
}

/** ===================== POLLING (АВТОЗАГРУЗКА ИСТОРИИ) ===================== */
function enablePolling() {
  ScriptApp.getProjectTriggers().forEach(t => {
    if (t.getHandlerFunction() === 'pollHistoryEvery5Min') {
      ScriptApp.deleteTrigger(t);
    }
  });

  ScriptApp.newTrigger('pollHistoryEvery5Min')
    .timeBased()
    .everyMinutes(5)
    .create();

  SpreadsheetApp.getUi().alert('✅ Автозагрузка включена!\n\nСМС будут автоматически загружаться каждые 5 минут через API истории.\n\nЭто работает даже если webhook не работает.');
}

function pollHistoryEvery5Min() {
  try {
    ensureSheets_();
    const props = PropertiesService.getScriptProperties();
    const lastCheck = props.getProperty('LAST_POLL_TIME');
    const since = lastCheck || new Date(Date.now() - 10 * 60 * 1000).toISOString();

    const oldSince = props.getProperty('HISTORY_SINCE');
    const oldDirection = props.getProperty('HISTORY_DIRECTION');

    props.setProperty('HISTORY_SINCE', since);
    props.setProperty('HISTORY_DIRECTION', 'incoming');

    backfillHistoryMTT();

    if (oldSince) props.setProperty('HISTORY_SINCE', oldSince);
    else props.deleteProperty('HISTORY_SINCE');

    if (oldDirection) props.setProperty('HISTORY_DIRECTION', oldDirection);
    else props.deleteProperty('HISTORY_DIRECTION');
...
function disablePolling() {
  ...
  SpreadsheetApp.getUi().alert(`⏹️ Автозагрузка выключена (удалено триггеров: ${removed})`);
}
