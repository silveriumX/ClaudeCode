"""
VPS Connection Utility for TelegramHub
========================================
...
REMOTE_DIR = os.getenv("VPS_REMOTE_DIR", "/root/universal_cursor_bot")
PROCESS_GREP = "universal_cursor_bot/bot.py"

CORE_FILES = [
    "server/main.py",
    "server/config.py",
    "server/telegram_manager.py",
    "server/universal_cursor_bot.py",
    "server/mobile_cursor_bot.py",
]
...
def upload_file(local_name: str, remote_path: str | None = None) -> None:
    ...
    try:
        ...
        run_ssh(f"mkdir -p {rdir}", print_output=False)
...
def bot_status():
    print("=== TelegramHub Processes ===")
    run_ssh(f"ps aux | grep -E 'universal_cursor_bot' | grep -v grep || echo 'Not running'")
    print("\n=== Uptime / RAM / Disk ===")
...
def bot_restart():
    print("=== Stopping ===")
    run_ssh(f"pkill -f 'universal_cursor_bot' 2>/dev/null; sleep 2")
    print("=== Starting ===")
    run_ssh(
        f"cd {REMOTE_DIR} && "
        f"source /root/voice_bot/venv/bin/activate 2>/dev/null; "
        f"nohup /root/voice_bot/venv/bin/python3 {REMOTE_DIR}/bot.py >> bot.log 2>> bot_error.log </dev/null & "
        f"sleep 2 && ps aux | grep 'universal_cursor_bot' | grep -v grep && echo 'Started OK' || echo 'FAILED'"
    )
