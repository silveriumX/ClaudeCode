...
## Contract-First для функций с побочными эффектами

Перед написанием функции которая пишет в БД, файл, внешний API или меняет внешнее
состояние — обязательно ответить в docstring на 3 вопроса:

1. **Что меняется?** — какие поля/ячейки/файлы
2. **Что НЕ меняется?** — инварианты (что не трогаем никогда)
3. **Что при ошибке?** — `False`? `None`? исключение?
...
**Правило:** docstring с Side effects/Invariants пишется **до** реализации тела функции.
Если не можешь сформулировать контракт — уточни задачу перед кодингом.
...
- [ ] **Contract-first для функций с побочными эффектами** (Side effects / Invariants)
